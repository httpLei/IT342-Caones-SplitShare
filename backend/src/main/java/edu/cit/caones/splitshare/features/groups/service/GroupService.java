package edu.cit.caones.splitshare.features.groups.service;

import edu.cit.caones.splitshare.shared.dto.request.CreateGroupRequest;
import edu.cit.caones.splitshare.shared.dto.request.UpdateGroupRequest;
import edu.cit.caones.splitshare.shared.dto.response.ExpenseDto;
import edu.cit.caones.splitshare.shared.dto.response.GroupDetailsDto;
import edu.cit.caones.splitshare.shared.dto.response.GroupMemberBalanceDto;
import edu.cit.caones.splitshare.shared.dto.response.GroupSummaryDto;
import edu.cit.caones.splitshare.features.groups.entity.BalanceSettlement;
import edu.cit.caones.splitshare.features.groups.entity.Expense;
import edu.cit.caones.splitshare.features.groups.entity.Group;
import edu.cit.caones.splitshare.features.groups.entity.GroupMember;
import edu.cit.caones.splitshare.features.auth.entity.User;
import edu.cit.caones.splitshare.features.groups.repository.BalanceSettlementRepository;
import edu.cit.caones.splitshare.features.groups.repository.ExpenseRepository;
import edu.cit.caones.splitshare.features.groups.repository.GroupRepository;
...
public class GroupService {
    ...
    private GroupMember createGroupMember(User user) {
        return GroupMember.builder().user(user).build();
    }
    ...
    private GroupDetailsDto toDetailsDto(Group group, String currentUserEmail) {
        ...
        return GroupDetailsDto.builder()
                ...
                .name(group.getName())
                .members(group.getMembers().stream().map(member -> member.getUser().getFirstname() + " " + member.getUser().getLastname()).toList())
                .memberEmails(group.getMembers().stream().map(member -> member.getUser().getEmail()).toList())
                .total(total)
                .balance(netBalance)
                .createdAt(group.getCreatedAt())
                .balances(group.getMembers().stream().map(member -> {
                ...
                .expenses(expenses.stream().map(expense -> toExpenseDto(expense, balances, currentUserEmail)).toList())
                .build();
    }
    ...
    private Set<String> normalizeEmails(List<String> emails) {
        ...
        return emails.stream().map(String::trim).filter(email -> !email.isBlank()).map(String::toLowerCase).collect(Collectors.toSet());
    }
    ...
    private ExpenseDto toExpenseDto(Expense expense, Map<String, BigDecimal> balances, String currentUserEmail) {
        ...
        BigDecimal impact = payerEmail.equalsIgnoreCase(currentUserEmail) ? expense.getAmount().subtract(share) : share.negate();
    ...
    private BigDecimal sumExpenses(List<Expense> expenses) {
        return expenses.stream().map(Expense::getAmount).reduce(BigDecimal.ZERO, BigDecimal::add).setScale(2, RoundingMode.HALF_UP);
    }
