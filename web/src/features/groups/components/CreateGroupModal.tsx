      ...
      <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-gray-800 p-6 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Start a shared expense group.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Mutual users to include</label>
            <div className="max-h-48 overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-900 p-3">
              {mutuals.length === 0 ? (
                <p className="text-xs text-gray-500 dark:text-gray-400">No mutual users yet. If you only followed someone, ask them to follow you back first.</p>
              ) : (
            ...
            <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">Only users with mutual follow relationships can be added.</p>
          </div>
