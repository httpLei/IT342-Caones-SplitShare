      ...
      <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-gray-800 p-6 shadow-2xl" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between gap-4 mb-5">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Rename the group or add more mutual members.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Group name</label>
            <input
              required
              value={name}
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Current members</label>
            <div className="rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-900 p-3">
              {currentMembers.length === 0 ? (
                <p className="text-xs text-gray-500 dark:text-gray-400">No members available.</p>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {currentMembers.map((member) => (
                    <span key={member.email || member.name} className="rounded-full bg-gray-100 dark:bg-gray-700 px-3 py-1 text-xs font-medium text-gray-700 dark:text-gray-200">
                      {member.name}
                    </span>
                  ))}
                </div>
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Add more mutual users</label>
            <div className="max-h-48 overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-900 p-3">
              {mutuals.length === 0 ? (
                <p className="text-xs text-gray-500 dark:text-gray-400">No mutual users yet.</p>
              ) : (
                <div className="space-y-2">
                  {mutuals.map((user) => (
                    <label key={user.id} className="flex items-center gap-3 rounded-lg px-2 py-1.5 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer">
                      <input
                        type="checkbox"
            ...
            <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">Only mutual users can be added to the group.</p>
          </div>
